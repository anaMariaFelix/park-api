...
import com.anamaria.park_api.service.ClienteVagaService;
import com.anamaria.park_api.service.EstacionamentoService;
import com.anamaria.park_api.service.JasperService;
...
import com.anamaria.park_api.web.controller.dto.mapper.ClienteVagaMapper;
...
public class EstacionamentoController {
    ...
    public ResponseEntity<EstacionamentoResponseDTO> checkin(@RequestBody @Valid EstacionamentoCreateDTO estacionamentoCreateDTO){
        ...
        URI location = ServletUriComponentsBuilder
                .fromCurrentRequestUri().path("/{recibo}")// recupera a uri de acesso que foi usada para criar a vaga 'api/v1/vagas'
                .buildAndExpand(clienteVaga.getRecibo())//inclui o codigo da vaga na uri
                .toUri();
    ...
    public ResponseEntity<Void> getRelatorio(HttpServletResponse response, @AuthenticationPrincipal JwtUserDetails user) throws IOException {
        ...
        jasperService.addParams("CPF", cpf);//parametro que existe no relatorio q é o cpf, ai passa o cpf dp cliente

        byte[] bytes = jasperService.gerarPdf();

        response.setContentType(MediaType.APPLICATION_PDF_VALUE);         //o nome do relatorio pode ser qualquee valor no caso aqui foi usado System.currentTimeMillis() a data de criação
        response.setHeader("Content-disposition", "inline; filename=" + "relatorioGerado" + System.currentTimeMillis() + ".pdf");
